...
def maiqwn():
    ...
    /load - obtener velocidad de carga (últimas 5 por desk)""",
        ...
        async def setsite(message: telebot.types.Message):
            ...
            if sites:
                ...
                await bot.send_message(
                    message.chat.id,
                    f"Actualmente, no hay ningún Sitio. Añada con /newsite.",
                )
        ...
        async def load(message: telebot.types.Message):
            ...
            if not q:
                await bot.send_message(
                    message.chat.id,
                    "No hay ninguna Métrica de Velocidad en ningún Desk.",
                )
                return
            ...
            for desk in desks:
                ...
                average_transmission_speed = total_transmission_speed / len(
                    measurements
                )
    ...
    Si decides cambiar de opinión, estoy aquí para ayudarte en cualquier momento.""",
        ...
        async def capture_desk_name_for_newdesk(message: telebot.types.Message):
            ...
            await bot.send_message(
                message.chat.id, f"El nombre del desk es: {desk_name}."
            )
